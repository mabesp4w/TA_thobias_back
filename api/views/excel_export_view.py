...
class ExcelExportMixin:
    ...
    def export_sales_report(self, request):
        ...
        for idx, sale in enumerate(queryset, 1):
            ...
            # Add UMKM name if admin
            if request.user.is_staff:
                umkm_name = sale.produk.umkm.get_full_name() if sale.produk and sale.produk.umkm else '-'
        ...
        if queryset.exists():
            ...
            total_col = 8 if request.user.is_staff else 7
    ...
    def export_sales_analysis(self, request):
        ...
        # Product sales data - include UMKM info if admin
        if request.user.is_staff:
            product_sales = base_queryset.values(
                'produk__nm_produk',
                'produk__kategori__nm_kategori',
                'produk__umkm__first_name',
                'produk__umkm__last_name'
            ).annotate(
                total_quantity=Sum('jumlah_terjual'),
                total_revenue=Sum('total_penjualan'),
        ...
        for idx, product in enumerate(product_sales, 2):
            ...
            if request.user.is_staff:
                umkm_name = f"{product.get('produk__umkm__first_name', '')} {product.get('produk__umkm__last_name', '')}".strip() or '-'
